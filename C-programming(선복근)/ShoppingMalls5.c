...
static int print_menu(void) {
    int menu = 0;
    printf("\n원하는 메뉴를 선택하시오.\n");
    printf("(1. 입고, 2. 판매, 3. 개별현황, 4. 전체현황, 5. 종료, 6.데이터 파일로 저장, 7. 파일 로드&출력)\n");
    printf("메뉴: ");
    if (scanf("%d", &menu) != 1) {
        // 잘못된 입력 버퍼 비우기
        int ch;
...
void saveData(const  ITEM* arr, int n, const char* filename) {
...
void loadData(ITEM* arr, int n, const char* filename) {
...
void export_to_text(const ITEM* arr, int n, const char* filename) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        fprintf(fp, "ID:%d | 상품명:%s | 입고량:%.2f | 판매량:%.2f | 판매가격:%.2f\n",
			arr[i].id, arr[i].name, arr[i].stocked, arr[i].sold, arr[i].price);
    }
    fclose(fp);
}
void print_exported_text(const char* filename) {
...
int main(void) {
    ...
    while (1) {
        ...
        case 6: {
            ...
            scanf("%99s", filename);
            saveData(items, quantity, filename);
        ...
        case 7: {
            ...
            scanf("%99s", filename);
            loadData(items, quantity, filename);
			printf("데이터가 파일에서 불러와졌습니다: %s\n", filename);
			export_to_text(items, quantity, "exported_data.txt");
            printf("불러온 데이터가 exported_data.txt 파일로 내보내졌습니다.\n");
			print_exported_text("exported_data.txt");
            break;
        }
        default:
            fprintf(stderr, "잘못된 입력입니다(1~5 중 입력).\n");
